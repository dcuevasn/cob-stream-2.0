...
import { Check, X, Loader2, AlertCircle, Pause } from 'lucide-react';
import { cn } from '../../lib/utils';
import { useStreamStore } from '../../hooks/useStreamStore';
import type { LaunchProgressStatus, PauseProgressItem, SecurityType } from '../../types/streamSet';
...
/** Get status icon for a pause progress item */
function StatusIcon({ status }: { status: LaunchProgressStatus }) {
  switch (status) {
    ...
    case 'processing':
      return <Loader2 className="w-3 h-3 text-red-400 animate-spin" />;
    case 'success':
      return (
        <div className="w-3 h-3 rounded-full bg-red-500 flex items-center justify-center animate-scale-in">
          <Pause className="w-2 h-2 text-white" strokeWidth={3} />
        </div>
      );
    case 'error':
      return (
        <div className="w-3 h-3 rounded-full bg-yellow-500 flex items-center justify-center animate-shake">
    ...
    >
      ...
      <span
        className={cn(
          ...
          status === 'processing' && 'shimmer-text-red'
        )}
      >
        {streamName}
      </span>
      {status === 'processing' && (
        <span className="text-red-400 animate-pulse-dots">...</span>
      )}
      {status === 'success' && (
        <span className="text-red-500/70 text-[10px] tabular-nums whitespace-nowrap">
          ({bidCount} bid, {askCount} ask)
        </span>
      )}
      {status === 'error' && (
        <span className="text-yellow-400/70 text-[10px] tabular-nums whitespace-nowrap" title={error}>
          ({bidCount} bid, {askCount} ask)
        </span>
      )}
    </div>
...
function groupItemsBySecurityType(items: PauseProgressItem[]) {
  const groups = new Map<SecurityType, PauseProgressItem[]>();
...
/** Main popover component for pause progress */
export function PauseProgressPopover() {
  const pauseProgress = useStreamStore((s) => s.pauseProgress);
  const dismissPauseProgress = useStreamStore((s) => s.dismissPauseProgress);
  const listRef = useRef<HTMLDivElement>(null);
  
  // Group items by security type (only when in All view with multiple types)
  const groupedItems = useMemo(() => {
    if (!pauseProgress) return null;
    const uniqueTypes = new Set(pauseProgress.items.map((item) => item.securityType));
    // Only group if there are multiple security types (All view)
    if (uniqueTypes.size <= 1) return null;
    return groupItemsBySecurityType(pauseProgress.items);
  }, [pauseProgress?.items]);
  
  // Auto-scroll to show latest processing item
  useEffect(() => {
    if (listRef.current && pauseProgress) {
      const processingIndex = pauseProgress.items.findIndex((item) => item.status === 'processing');
      if (processingIndex >= 0) {
        // Find the actual DOM element (accounting for section headers)
        const processingItem = listRef.current.querySelector(`[data-stream-id="${pauseProgress.items[processingIndex].streamId}"]`);
        if (processingItem) {
  ...
  const { items, completedCount, totalCount, variant, isActive } = pauseProgress;
      ...
      >
        {groupedItems ? (
          ...
          groupedItems.map((group, groupIndex) => {
            ...
            // Calculate base index for animation delay continuity
